...
# --------------------------------------------------------------
# SECTION 6 — PLOTTING (unchanged except cleaned)
# --------------------------------------------------------------
...
ax.plot(np.log10(mass_pl), pl_vals, 'r', lw=3, label="Giant Planet Model")
ax.plot(np.log10(mass_bd), bd_vals, 'b', lw=3, label="Brown Dwarf Model")
ax.plot(np.log10(mass_total), total_vals, 'orange', lw=2, label="Total Frequency")
...
# --------------------------------------------------------------
# SECTION 7 — FREQUENCY CALCULATOR (unchanged)
# --------------------------------------------------------------

def f_pl(mmin, mmax, amin, amax, Mhost):
    qmin = (mmin*0.001)/Mhost
    qmax = (mmax*0.001)/Mhost
    return integrate.quad(mass_fctn_pl, qmin, qmax)[0]*orb_pl

def f_bd(mmin, mmax, amin, amax, Mhost):
    qmin = (mmin*0.001)/Mhost
    qmax = (mmax*0.001)/Mhost
    return integrate.quad(mass_fctn_bd, qmin, qmax)[0]*orb_bd
